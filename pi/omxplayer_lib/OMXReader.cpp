...
{
  //printf("<<OMXReader.cpp: interrupt_cb\n");
  if(g_abort)
    return 1;
  return 0;
}
...
OMXPacket *OMXReader::Read()
{
  ...
  result = m_dllAvFormat.av_read_frame(m_pFormatContext, &pkt); //printf("av_read_frame\n"); //tvrain
...
{
  printf(">>OMXReader::SetActiveStream()\n");
  bool ret = false;
  Lock();
